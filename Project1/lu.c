// NAME: KALLIOPI-CHRISTINA DESPOTIDOU AM: 1115202000045
...
int main(int argc, char *argv[])
{
    ...
    if (atoi(argv[1]) == 1)
    { // otan o xristis thelei na eisagei monos tou ta dedomena, eisagei kai ton pinaka A kai to dianisma b
    ...
    { // otan theloume tixaio pinaka, gia eukoleia stis prakseis evala anotato orio to 100
    ...
    { // otan theloume na diavasei apo arxeio, sto arxeio prepei na nai se grammes ksexorista ta stoixeia
        char *X;
        X = argv[2];
...
double calc_vector_norm(double **x, int n, int flag)
{ // flag=1-> norm tou x, flag=0, norm tou b(to dianisma einai stili enos pinaka)
    double maximum;
    double temp[n];
    for (int i = 0; i < n; i++)
    {
...
void substract_vectors(double **first, double **second, int n, double **diff, int flag)
{ // flag=1->x, flag=0->b(to dianisma einai stili enos pinaka)
    for (int i = 0; i < n; i++)
    {
        if (flag == 1)
