...
while(itcount < max_iterations)
    ...
    for i = 1:n
        if i > 2 && i <= n-2
            temp(i) = (1-t_opt)*solution(i) + w_opt*dot(-A(i,i-2:i-1)/A(i,i), temp(i-2:i-1)') + (t_opt-w_opt)*dot(-A(i,i-2:i-1)/A(i,i),solution(i-2:i-1)') + t_opt*dot(-A(i,i+1:i+2)/A(i,i),solution(i+1:i+2)') + (t_opt*b(i)/A(i,i));
        elseif i <=2
            temp(i) = (1-t_opt)*solution(i) + w_opt*dot(-A(i,1:i-1)/A(i,i), temp(1:i-1)') + (t_opt-w_opt)*dot(-A(i,1:i-1)/A(i,i),solution(1:i-1)') + t_opt*dot(-A(i,i+1:i+2)/A(i,i),solution(i+1:i+2)') + (t_opt*b(i)/A(i,i));
        elseif i == n-1
            temp(i) = (1-t_opt)*solution(i) + w_opt*dot(-A(i,i-2:i-1)/A(i,i), temp(i-2:i-1)') + (t_opt-w_opt)*dot(-A(i,i-2:i-1)/A(i,i),solution(i-2:i-1)') + t_opt*dot(-A(i,n)/A(i,i),solution(i+1)) + (t_opt*b(i)/A(i,i));
        else
            temp(i) = (1-t_opt)*solution(i) + w_opt*dot(-A(i,i-2:i-1)/A(i,i), temp(i-2:i-1)') + (t_opt-w_opt)*dot(-A(i,i-2:i-1)/A(i,i),solution(i-2:i-1)') + (t_opt*b(i)/A(i,i));
        end
    end
    itcount = itcount+1;
...
fprintf("ESOR Execution Time: "); disp(endtime);
